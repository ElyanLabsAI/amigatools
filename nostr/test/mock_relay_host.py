...
Identical to the original except the listen socket is bound to 0.0.0.0
instead of 127.0.0.1, so it is reachable from inside the FS-UAE guest via
bsdsocket_library's host-loopback passthrough (the guest connects to
127.0.0.1:8813, which lands on this 0.0.0.0:8813 listener on the host).
Kept as a separate file rather than editing the shared tools/mock_relay.py
used by `make demo`.
...
Usage:  python3 mock_relay_host.py [port]   (default 7000)
"""
